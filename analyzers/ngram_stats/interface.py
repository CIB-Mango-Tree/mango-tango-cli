...
from ..ngrams.interface import (COL_AUTHOR_ID, COL_MESSAGE_ID,
                                COL_MESSAGE_NGRAM_COUNT, COL_MESSAGE_TEXT,
                                COL_MESSAGE_TIMESTAMP, COL_NGRAM_ID,
...
interface = SecondaryAnalyzerInterface(
  ...
  outputs=[
    ...
    AnalyzerOutput(
      ...
      columns=[
        OutputColumn(
          name=COL_NGRAM_ID, data_type="identifier",
          human_readable_name="ngram ID"
        ),
        OutputColumn(
          name=COL_NGRAM_LENGTH, data_type="integer",
          human_readable_name="ngram length"
        ),
        OutputColumn(
          name=COL_NGRAM_WORDS, data_type="text",
          human_readable_name="ngram content"
        ),
        OutputColumn(
          name=COL_NGRAM_TOTAL_REPS, data_type="integer",
          human_readable_name="ngram frequency"
        ),
        OutputColumn(
          name=COL_NGRAM_DISTINCT_POSTER_COUNT, data_type="integer",
          human_readable_name="distinct user count"
        ),
        OutputColumn(
          name=COL_AUTHOR_ID, data_type="identifier",
        ...
        OutputColumn(
          name=COL_MESSAGE_SURROGATE_ID, data_type="identifier",
          human_readable_name="UPN"
        ),
        OutputColumn(
          name=COL_MESSAGE_ID, data_type="identifier",
          human_readable_name="post identifier"
        ),
        OutputColumn(
          name=COL_MESSAGE_TEXT, data_type="text",
          human_readable_name="post content"
        ),
        OutputColumn(
          name=COL_MESSAGE_TIMESTAMP, data_type="datetime",
